...
execute_lambda_function() {
	...
	time output=$(aws lambda invoke --cli-read-timeout 600 --invocation-type RequestResponse --function-name arm-function$1 --region us-east-2 --cli-binary-format raw-in-base64-out --payload "$json" /dev/stdout | head -n 1 | head -c -2)
	local ret=$?
	if [ $ret -ne 0 ]; then
		echo "ERROR: Stopping workflow, something bad happened!"
	else
		echo $output | jq | tee $mydir/$mystart-cold-arm-function$1.txt
	fi
	return $ret
}
