...
execute_lambda_function() {
	...
	time output=$(aws lambda invoke --cli-read-timeout 600 --invocation-type RequestResponse --function-name tcss562_term_project --region us-east-2 --cli-binary-format raw-in-base64-out --payload "$json" /dev/stdout)
	local ret=$?
	if [[ $ret -ne 0 || -z "$output" ]]; then
		echo "ERROR: Stopping workflow, something bad happened!"
	else
		echo $output | head -n 1 | head -c -2 | jq | tee $mydir/$mystart-warm-x86-function$1.txt
	fi
	return $ret
}
